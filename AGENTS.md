...
`config.yaml` (precedence: explicit `LOREKEEP_*` env > `LOREKEEP_HOME` > dev marker > XDG). **API keys never go in committed files** — use `provider.api_key_env` (name of an env var); inline `provider.api_key` is allowed only in the gitignored local config. `compile.language` is a lowercase ISO 639-1 code that controls the language of LLM-extracted names, summaries, and descriptions and defaults to `en`; source Markdown and technical identifiers are not rewritten. Serve-time scope comes from `LOREKEEP_NS` (comma-separated) env or `config.ns.default`. **Namespace patterns support wildcards**: `*` (all graph namespaces), `*-session` (glob match), `me` (literal). Patterns are expanded against the graph's actual namespaces in `_rebuild()`, so new namespaces from recompile are picked up on lazy reload. Default config: `ns.default: [me, "*-session", "*-memory"]`. Template: `.lorekeep/config.yaml.example`.
