...
def test_namespace_from_path(tmp_path: Path):
    raw = tmp_path / "raw"
    f = raw / "backend" / "payments.md"
    f.parent.mkdir(parents=True)
    f.write_text("x\n")
    assert namespace_for(raw, f) == "backend"
...
def test_ingest_splits_into_line_chunks(tmp_path: Path):
    ...
    assert chunks[0].namespace == "backend"
    assert chunks[0].src == "backend/a.md:1"
    assert chunks[1].start_line == 61


def test_ingest_sorted_and_skips_dirs(tmp_path: Path):
    raw = tmp_path / "raw"
    (raw / "backend").mkdir(parents=True)
    (raw / "backend" / "b.md").write_text("b\n")
    (raw / "backend" / "a.md").write_text("a\n")
    chunks = ingest(raw)
