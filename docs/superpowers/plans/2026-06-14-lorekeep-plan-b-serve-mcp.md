# Lorekeep Plan B — Serve (Store + Permission + MCP + Integrations) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Serve the compiled temporal knowledge graph read-only to coding agents (Claude Code, Cursor, Codex) over MCP (stdio-first), with per-namespace permission enforced at the query layer, plus `lorekeep mcp add` integration and a minimal Tier-2 retrieval/temporal eval harness.
...
│   └── codex.py              # write config.toml [mcp_servers.lorekeep]
└── eval/
...
**Files:**
- Create: `src/lorekeep/facts_io.py`
- Modify: `src/lorekeep/eval/gold.py` (use the shared loader)

- [ ] **Step 1: Write `src/lorekeep/facts_io.py`**
...
- [ ] **Step 2: Refactor `src/lorekeep/eval/gold.py` to use it**
...
```bash
git add src/lorekeep/facts_io.py src/lorekeep/eval/gold.py
...
**Files:**
- Create: `src/lorekeep/store/__init__.py` (empty)
- Create: `src/lorekeep/store/graph.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'lorekeep.store'`).

- [ ] **Step 3: Create empty `src/lorekeep/store/__init__.py`**

- [ ] **Step 4: Write `src/lorekeep/store/graph.py`**
...
```bash
git add src/lorekeep/store/__init__.py src/lorekeep/store/graph.py tests/test_graph_store.py
...
**Files:**
- Modify: `src/lorekeep/store/graph.py` (add `out_edges`, `in_edges`, `neighbors`)
...
- [ ] **Step 3: Append to `src/lorekeep/store/graph.py`** (inside the class, after `all_edges`)
...
```bash
git add src/lorekeep/store/graph.py tests/test_graph_store.py
...
**Files:**
- Modify: `src/lorekeep/store/graph.py` (add `_active`, `snapshot`)
- Test: `tests/test_graph_store.py` (append)
...
- [ ] **Step 3: Append to `src/lorekeep/store/graph.py`**
...
**Files:**
- Modify: `src/lorekeep/store/graph.py` (add `history`, `changes`)
- Test: `tests/test_graph_store.py` (append)
...
**Files:**
- Create: `src/lorekeep/perm/__init__.py` (empty)
- Create: `src/lorekeep/perm/ns.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'lorekeep.perm'`).

- [ ] **Step 3: Create empty `src/lorekeep/perm/__init__.py`**

- [ ] **Step 4: Write `src/lorekeep/perm/ns.py`** (rules only — `ScopedGraph` comes in Task 8)
...
```bash
git add src/lorekeep/perm/__init__.py src/lorekeep/perm/ns.py tests/test_perm.py
...
**Files:**
- Modify: `src/lorekeep/perm/ns.py` (add `ScopedGraph`)
...
- [ ] **Step 3: Append `ScopedGraph` to `src/lorekeep/perm/ns.py`**
...
```bash
git add src/lorekeep/perm/ns.py tests/test_perm.py
...
**Files:**
- Modify: `src/lorekeep/perm/ns.py` (add methods)
- Test: `tests/test_perm.py` (append)
...
- [ ] **Step 3: Append to `ScopedGraph` in `src/lorekeep/perm/ns.py`**
...
**Files:**
- Create: `src/lorekeep/store/fts.py`
...
- [ ] **Step 3: Write `src/lorekeep/store/fts.py`**
...
```bash
git add src/lorekeep/store/fts.py tests/test_fts.py
...
**Files:**
- Modify: `src/lorekeep/store/graph.py` (add `search`)
- Modify: `src/lorekeep/perm/ns.py` (add `ScopedGraph.search`)
...
- [ ] **Step 4: Append to `GraphStore` in `src/lorekeep/store/graph.py`**
...
- [ ] **Step 5: Append to `ScopedGraph` in `src/lorekeep/perm/ns.py`**
...
```bash
git add src/lorekeep/store/graph.py src/lorekeep/perm/ns.py tests/test_graph_store.py tests/test_perm.py
...
**Files:**
- Create: `src/lorekeep/mcp_server.py`
...
- [ ] **Step 3: Write `src/lorekeep/mcp_server.py`**
...
```bash
git add src/lorekeep/mcp_server.py tests/test_mcp_server.py
...
**Files:**
- Modify: `src/lorekeep/mcp_server.py` (add `at_time`, `history`, `changes`, `search`)
...
- [ ] **Step 3: Append to `src/lorekeep/mcp_server.py`** (before the `if __name__` block)
...
**Files:**
- Modify: `src/lorekeep/cli.py` (add `serve`)
- Modify: `src/lorekeep/config.py` (add `ns` field)
- Test: `tests/test_serve_cli.py`

- [ ] **Step 1: Add `ns` to config. Modify the `Config` model in `src/lorekeep/config.py`**
...
(Define `NsConfig` before `Config`. The `default` is `["public"]` so a caller with no `LOREKEEP_NS` sees only public facts.)
...
- [ ] **Step 4: Append `serve` to `src/lorekeep/cli.py`** (before the `if __name__` block)
...
```bash
git add src/lorekeep/cli.py src/lorekeep/config.py tests/test_serve_cli.py
...
**Files:**
- Create: `src/lorekeep/integrations/__init__.py` (empty)
- Create: `src/lorekeep/integrations/common.py`
- Create: `src/lorekeep/integrations/claude_code.py`
- Create: `src/lorekeep/integrations/cursor.py`
- Create: `src/lorekeep/integrations/codex.py`
...
- [ ] **Step 3: Create empty `src/lorekeep/integrations/__init__.py`**

- [ ] **Step 4: Write `src/lorekeep/integrations/common.py`**
...
- [ ] **Step 5: Write `src/lorekeep/integrations/claude_code.py`**
...
- [ ] **Step 6: Write `src/lorekeep/integrations/cursor.py`**
...
- [ ] **Step 7: Write `src/lorekeep/integrations/codex.py`**
...
def write_config(target_dir: Path, command: str, args: list[str], ns: str | None) -> Path:
    ...
    if ns:
        lines.append(f'env = {{ LOREKEEP_NS = "{ns}" }}')
...
```bash
git add src/lorekeep/integrations/ tests/test_integrations.py
...
**Files:**
- Modify: `src/lorekeep/config.py` (add `install_source`)
- Modify: `src/lorekeep/cli.py` (add `mcp` subgroup with `add`)
- Test: `tests/test_mcp_add_cli.py`

- [ ] **Step 1: Add `install_source` to `Config` in `src/lorekeep/config.py`**
...
- [ ] **Step 4: Add the `mcp` subgroup to `src/lorekeep/cli.py`** (before the `if __name__` block)
...
```bash
git add src/lorekeep/config.py src/lorekeep/cli.py tests/test_mcp_add_cli.py
...
**Files:**
- Modify: `src/lorekeep/cli.py` (add `doctor`)
...
- [ ] **Step 3: Append `doctor` to `src/lorekeep/cli.py`** (before the `if __name__` block)
...
```bash
git add src/lorekeep/cli.py tests/test_doctor_cli.py
...
**Files:**
- Create: `src/lorekeep/eval/retrieval.py`
...
- [ ] **Step 4: Write `src/lorekeep/eval/retrieval.py`**
...
```bash
git add src/lorekeep/eval/retrieval.py tests/fixtures/retrieval/questions.json tests/test_retrieval_eval.py
...
```bash
LOREKEEP_PROVIDER=fake uv run lorekeep compile   # produces graph/facts.jsonl
```

## 2. Scope your namespace

Set `LOREKEEP_NS` to the namespaces the agent may read (comma-separated). Unset
defaults to `public` (only public facts).

```bash
export LOREKEEP_NS=teams/backend
```

## 3. Wire up a coding agent

```bash
uv run lorekeep mcp add --agent claude --ns teams/backend
uv run lorekeep mcp add --agent cursor
uv run lorekeep mcp add --agent codex  --ns teams/backend
```
...
`.cursorrules` / `AGENTS.md`. Set `install_source` in `.lorekeep/config.yaml`
...
Run:
```bash
LOREKEEP_CONFIG=/tmp/lorekeep-cfg.yaml sh -c 'echo "install_source: local" > /tmp/lorekeep-cfg.yaml'
LOREKEEP_CONFIG=/tmp/lorekeep-cfg.yaml uv run lorekeep mcp add --agent claude --ns teams/backend
LOREKEEP_NS=teams/backend uv run lorekeep doctor
```
...
- stdio transport, LOREKEEP_NS → allowed_ns, deny-default, edge-needs-both-endpoints → T7, T8, T14 ✓
