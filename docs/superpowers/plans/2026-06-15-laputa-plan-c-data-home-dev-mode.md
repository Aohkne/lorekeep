# Laputa Plan C — Data Home + Dev Mode Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Separate the Laputa tool from its data so `uvx laputa` installs clean (data in an XDG home by default) while keeping convenient local-dev (repo co-located data, zero migration).

**Architecture:** A new `paths.py` provides `resolve_paths()` with 4-tier precedence: (1) explicit per-path env overrides `LAPUTA_RAW/OUT/CACHE/SCHEMA/CONFIG` (tests + power users), (2) `LAPUTA_HOME` → unified `<home>/{config.yaml,schema.json,raw,graph,cache.json}`, (3) dev mode (`.laputa/` or `raw/` in CWD, or `LAPUTA_DEV=1`) → current CWD layout unchanged, (4) default → XDG via `platformdirs`. CLI commands call `resolve_paths()` instead of the old `_paths()`; the returned dict shape is identical so command bodies don't change. A new `laputa init` bootstraps the home with default config + schema. `mcp add` already emits a portable `.mcp.json` when `install_source` is `pypi`/`local`.

**Tech Stack:** Python 3.11+, platformdirs (new dep), typer, pydantic, uv.

**Spec:** `docs/superpowers/specs/2026-06-14-laputa-temporal-kg-mcp-design.md`. **Prior plans:** A (compile) + B (serve) merged (95 tests green).

**Current `_paths()` (cli.py:25-32)** reads `LAPUTA_*` env with CWD defaults — this is exactly dev mode. The refactor generalizes the default (dev/home/xdg) while preserving the env-override tier.
...
.laputa/config.yaml.example  # MODIFY: install_source note (pypi for portable)
tests/
├── test_paths.py     # NEW
├── test_defaults.py  # NEW
├── test_init_cli.py  # NEW
└── (all existing tests unchanged — they use LAPUTA_* env = tier 1)
```
...
```bash
uv sync
uv run python -c "import platformdirs; print(platformdirs.user_config_dir('laputa'), platformdirs.user_data_dir('laputa'))"
```
Expected: prints two paths (e.g. `~/.config/laputa ~/.local/share/laputa`), no ImportError.
...
**Files:** Create `src/laputa/paths.py`, Test `tests/test_paths.py`
...
def test_laputa_home_overrides_dev(tmp_path: Path, monkeypatch):
    # even inside a dev-marked dir, explicit LAPUTA_HOME wins
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".laputa").mkdir()
...
def test_xdg_default(tmp_path: Path, monkeypatch):
    # no dev marker, no LAPUTA_HOME -> XDG (redirect via XDG_*_HOME for determinism)
...
Expected: FAIL (`ModuleNotFoundError: No module named 'laputa.paths'`).

- [ ] **Step 3: Write `src/laputa/paths.py`**
...
```bash
git add src/laputa/paths.py tests/test_paths.py
...
**Files:** Modify `src/laputa/cli.py`

- [ ] **Step 1: Add the import** — at the top of `src/laputa/cli.py`, add after the existing laputa imports (after `from laputa.schema_io import load_schema`):

```python
from laputa.paths import resolve_paths
```

- [ ] **Step 2: Remove the `_paths()` function** — delete the entire `def _paths() -> dict[str, Path]:` block (currently lines 25-32, the function returning the dict with `LAPUTA_*` env + CWD defaults).
...
Run: `uv run pytest -q`
Expected: 100 passed (95 existing + 5 new from Task 2). All existing CLI tests set `LAPUTA_*` env (tier 1) so they resolve identically; no command-body change.
...
uv run laputa check
```
Expected: running from the repo (which has `.laputa/`) → dev mode → compiles into repo `graph/`, `check` prints `ok`.

- [ ] **Step 6: Commit**

```bash
git add src/laputa/cli.py
...
**Files:** Create `src/laputa/defaults.py`, Test `tests/test_defaults.py`
...
Expected: FAIL (`ModuleNotFoundError: No module named 'laputa.defaults'`).

- [ ] **Step 3: Write `src/laputa/defaults.py`**
...
```bash
git add src/laputa/defaults.py tests/test_defaults.py
git commit -m "feat(defaults): DEFAULT_SCHEMA + DEFAULT_CONFIG_YAML for init"
```

---

## Task 5: `laputa init` command

**Files:** Modify `src/laputa/cli.py` (add `init`)
...
- [ ] **Step 3: Add the `init` command to `src/laputa/cli.py`** (before the `if __name__ == "__main__":` block). Also add the defaults import at the top:

Top of file, after `from laputa.paths import resolve_paths`:
```python
...
```bash
git add src/laputa/cli.py tests/test_init_cli.py
...
**Files:** Modify `docs/serve.md`, `docs/compile.md`, `.laputa/config.yaml.example`
...
- [ ] **Step 3: Update `.laputa/config.yaml.example`** — change the `install_source` comment line to clarify portability. Replace the last line:
...
```bash
git add docs/serve.md docs/compile.md .laputa/config.yaml.example
git commit -m "docs: data-home (dev/XDG/LAPUTA_HOME) + init + portable .mcp.json"
```
...
uv run laputa check
```
Expected: compiles into repo `graph/` (dev mode auto-detected via `.laputa/`); `check` prints `ok`.

- [ ] **Step 2: Home-mode smoke (isolated)**

Run:
```bash
LAPUTA_HOME=/tmp/laputa-smoke LAPUTA_PROVIDER=fake uv run laputa init
# seed a doc
mkdir -p /tmp/laputa-smoke/raw/teams/backend
cp raw/teams/backend/payments.md /tmp/laputa-smoke/raw/teams/backend/
LAPUTA_HOME=/tmp/laputa-smoke LAPUTA_PROVIDER=fake uv run laputa compile
LAPUTA_HOME=/tmp/laputa-smoke LAPUTA_NS=teams/backend uv run laputa doctor
```
Expected: `init` creates `/tmp/laputa-smoke/{config.yaml,schema.json,raw,graph}`; `compile` writes `/tmp/laputa-smoke/graph/facts.jsonl`; `doctor` prints `all checks passed: 4 nodes, 2 edges`.

- [ ] **Step 3: Portable `.mcp.json` smoke**

Run:
```bash
mkdir -p /tmp/laputa-mcp-smoke && cd /tmp/laputa-mcp-smoke
LAPUTA_HOME=/tmp/laputa-smoke LAPUTA_CONFIG=/tmp/laputa-smoke/config.yaml uvx --from /home/manhpt1/Workspace/laputa laputa mcp add --agent claude --ns teams/backend
cat .mcp.json
```
Expected: `.mcp.json` command is `uvx` with args `["laputa","serve","--transport","stdio"]` (NO `--from` / machine path, because `init` set `install_source: pypi`).

- [ ] **Step 4: Clean up smoke artifacts**

Run: `rm -rf /tmp/laputa-smoke /tmp/laputa-mcp-smoke`
...
- `laputa init` bootstrap → Task 5 ✓
- dev mode preserves current layout (zero migration) → Task 2 (dev branch) ✓
- portable `.mcp.json` (no machine path) when install_source=pypi → Task 6 + Task 7 smoke ✓ (resolve_command from Plan B already handles pypi→`uvx laputa serve`)
