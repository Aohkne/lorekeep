# Lorekeep Plan A — Compile Pipeline + Tier-1 Eval Implementation Plan
...
├── .lorekeep/config.yaml                  # LOCAL (.gitignored): provider + ns settings
├── src/lorekeep/
│   ├── __init__.py
│   ├── models.py                        # DocChunk, Node, Edge, Schema, Manifest (+ errors)
│   ├── config.py                        # load .lorekeep/config.yaml -> Config
│   ├── compile/
│   │   ├── __init__.py
...
│   └── cli.py                           # `lorekeep compile | eval | check` (typer)
├── tests/
...
- Create: `src/lorekeep/__init__.py`
- Create: `src/lorekeep/cli.py`
...
- [ ] **Step 2: Write `src/lorekeep/__init__.py`**
...
- [ ] **Step 3: Write `src/lorekeep/cli.py` (stub)**
...
Expected: FAIL — `ModuleNotFoundError: No module named 'lorekeep.models'`

- [ ] **Step 3: Write `src/lorekeep/models.py`**
...
```bash
git add src/lorekeep/models.py tests/test_models.py
...
**Files:**
- Create: `src/lorekeep/schema_io.py`
...
- [ ] **Step 3: Write `src/lorekeep/schema_io.py`**
...
```bash
git add src/lorekeep/schema_io.py tests/test_schema_io.py
git commit -m "feat(schema): JSON schema file loader"
```

---

## Task 4: Config loader (`.lorekeep/config.yaml`)

**Files:**
- Create: `src/lorekeep/config.py`
...
- [ ] **Step 3: Write `src/lorekeep/config.py`**

```python
"""Load local Lorekeep config. Lives in .lorekeep/config.yaml (gitignored)."""
from __future__ import annotations
...
```bash
git add src/lorekeep/config.py tests/test_config.py
git commit -m "feat(config): load .lorekeep/config.yaml with provider + compile settings"
```
...
**Files:**
- Create: `src/lorekeep/compile/__init__.py` (empty)
- Create: `src/lorekeep/compile/ingest.py`
...
- [ ] **Step 3: Write `src/lorekeep/compile/__init__.py`** (empty file)

- [ ] **Step 4: Write `src/lorekeep/compile/ingest.py`**
...
```bash
git add src/lorekeep/compile/__init__.py src/lorekeep/compile/ingest.py tests/test_ingest.py
...
**Files:**
- Create: `src/lorekeep/compile/providers.py`
...
- [ ] **Step 3: Write `src/lorekeep/compile/providers.py`**
...
```bash
git add src/lorekeep/compile/providers.py tests/test_providers.py
...
**Files:**
- Create: `src/lorekeep/compile/extract.py`
...
- [ ] **Step 3: Write `src/lorekeep/compile/extract.py`**
...
```bash
git add src/lorekeep/compile/extract.py tests/test_extract.py
...
**Files:**
- Modify: `src/lorekeep/compile/extract.py` (append cache + `extract_chunk`)
...
- [ ] **Step 3: Append to `src/lorekeep/compile/extract.py`**
...
**Files:**
- Create: `src/lorekeep/compile/resolve.py`
...
- [ ] **Step 3: Write `src/lorekeep/compile/resolve.py`**
...
```bash
git add src/lorekeep/compile/resolve.py tests/test_resolve.py
...
**Files:**
- Modify: `src/lorekeep/compile/resolve.py` (no signature change; covered already)
...
If any test fails, fix `resolve()` in `src/lorekeep/compile/resolve.py` until green. The current implementation assigns `e_{type}_{counter:04d}` in input order and quarantines dangling/self-loop.
...
```bash
git add tests/test_resolve.py src/lorekeep/compile/resolve.py
...
**Files:**
- Create: `src/lorekeep/compile/writer.py`
...
- [ ] **Step 3: Write `src/lorekeep/compile/writer.py`**
...
```bash
git add src/lorekeep/compile/writer.py tests/test_writer.py
...
**Files:**
- Create: `src/lorekeep/pipeline.py`
- Modify: `src/lorekeep/cli.py` (add `compile`)
...
- [ ] **Step 4: Write `src/lorekeep/pipeline.py`**
...
from lorekeep.models import Manifest, Node, Edge, Schema
...
def compile_graph(
    ...
    from lorekeep.models import Manifest as M
...
- [ ] **Step 5: Rewrite `src/lorekeep/cli.py`**
...
def _paths() -> dict[str, Path]:
    return {
        "raw": Path(os.environ.get("LOREKEEP_RAW", "raw")),
        "out": Path(os.environ.get("LOREKEEP_OUT", "graph")),
        "cache": Path(os.environ.get("LOREKEEP_CACHE", ".lorekeep/cache.json")),
        "schema": Path(os.environ.get("LOREKEEP_SCHEMA", "graph/schema.json")),
        "config": Path(os.environ.get("LOREKEEP_CONFIG", ".lorekeep/config.yaml")),
    }
...
```bash
git add src/lorekeep/pipeline.py src/lorekeep/cli.py tests/conftest.py tests/test_pipeline.py tests/test_compile_cli.py
...
**Files:**
- Create: `src/lorekeep/eval/__init__.py` (empty)
- Create: `src/lorekeep/eval/gold.py`
...
- [ ] **Step 3: Write `src/lorekeep/eval/__init__.py`** (empty)

- [ ] **Step 4: Write `src/lorekeep/eval/gold.py`**
...
```bash
git add src/lorekeep/eval/__init__.py src/lorekeep/eval/gold.py tests/test_construction_eval.py
...
**Files:**
- Create: `src/lorekeep/eval/construction.py`
...
- [ ] **Step 3: Write `src/lorekeep/eval/construction.py`**
...
```bash
git add src/lorekeep/eval/construction.py tests/test_construction_eval.py
...
**Files:**
- Modify: `src/lorekeep/eval/construction.py` (add `entity_resolution_f1`)
- Test: `tests/test_construction_eval.py` (append)
...
- [ ] **Step 3: Append to `src/lorekeep/eval/construction.py`**
...
**Files:**
- Modify: `src/lorekeep/eval/construction.py` (add `structure_report`)
- Test: `tests/test_construction_eval.py` (append)
...
**Files:**
- Modify: `src/lorekeep/cli.py` (add `eval` and `check`)
...
- [ ] **Step 3: Append two commands to `src/lorekeep/cli.py`** (before the `if __name__` block). The function is named `eval_cmd` with `name="eval"` to avoid shadowing the Python builtin `eval`.
...
```bash
git add src/lorekeep/cli.py tests/test_eval_cli.py
...
- Create: `.lorekeep/config.yaml.example`
- Modify: `.gitignore` (ensure `.lorekeep/` ignored)

- [ ] **Step 1: Ensure `.lorekeep/` is gitignored**

Run: `grep -q '\.lorekeep/' .gitignore || echo '.lorekeep/' >> .gitignore`

- [ ] **Step 2: Write `.lorekeep/config.yaml.example`**
...
uv run lorekeep check
LOREKEEP_GOLD=tests/fixtures/gold uv run lorekeep eval
```
...
```bash
git add docs/compile.md .lorekeep/config.yaml.example .gitignore
git commit -m "docs: compile quickstart + provider config example; ensure .lorekeep ignored"
```
