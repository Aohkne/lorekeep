...
The default MCP surface exposes these through
`temporal_query(mode="at_time"|"history"|"changes", params={...})`. The functions below
remain the internal graph operations and are also published as aliases by the
`full` profile.
