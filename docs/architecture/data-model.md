...
| `mcp_server` | store + perm | 7 core MCP tools + passive resources | FastMCP profiles, stdio (default). Loads store once; enforces permission per request. `full` retains legacy aliases; writes append to journals. |
